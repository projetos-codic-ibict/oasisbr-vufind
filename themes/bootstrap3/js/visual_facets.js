...
function settext () {
  this.text(function createText (d) {
    ...
    const onTop = (typeof d.parentfield === 'undefined')
    ...
    if (d.name === VuFind.translate('More Topics')) {
      const topics = VuFind.translate('more_topics')
...
function settitle () {
  this.attr('title', function createTitle (d) {
    ...
    if (typeof d.field !== 'undefined') {
      const on_topic = VuFind.translate('on_topic')
...
function showVisualFacets (pivotdata) {
  if (!d3.select('#visualResults').empty()) {
    ...
    const div = d3.select('#visualResults')
    ...
    const totalbooks = pivotdata.total
