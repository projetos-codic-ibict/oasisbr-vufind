...
(function autocomplete ($) {
  ...
  function Factory (_input, settings) {
    return function acClosure () {
      const input = $(this)
      let options = optionStorage[input.data('ac-id')]

      const _align = function _align () {
      ...
      const _populate = function _populate (item, eventType) {
      ...
      const _listToHTML = function _listToHTML (list, regex) {
        const shell = $('<div/>')
        for (let i = 0; i < list.length; i++) {
          if (typeof list[i] === 'string') {
            list[i] = { value: list[i] }
          }
          let content = list[i].label || list[i].value
          if (options.highlight) {
            content = content.replace(regex, '<b>$1</b>')
          }
          const item = typeof list[i].href === 'undefined' ? $('<div/>') : $('<a/>').attr('href', list[i].href)
          // list
          item
            .data(list[i])
            .addClass('ac-item')
      ...
      const _defaultStaticSort = function _defaultStaticSort (a, b) {
        return a.match.indexOf(this) - b.match.indexOf(this)
      }
      const _staticGroups = function _staticGroups (lcterm) {
        const matches = []
        function isTermMatch (_item) {
          return _item.match.match(lcterm)
        }
        for (let i = 0; i < options.static.groups.length; i++) {
          if (typeof options.static.groups[i].label !== 'undefined') {
            const mitems = options.static.groups[i].items.filter(isTermMatch)
            if (mitems.length > 0) {
            ...
            const ms = options.static.groups[i].filter(isTermMatch)
            if (ms.length > 0) {
              if (typeof options.staticSort === 'function') {
      ...
      function preprocessStatic (_item) {
        const item = typeof _item === 'string' ? { value: _item } : _item
        item.match = (item.label || item.value).toLowerCase()
        return item
      }
      const _setup = function _setup (settings) {
        ...
        input.keydown(function acinputKeydown (event) {
          ...
          let position = $(this).data('ac-selected')
          switch (event.which) {
      ...
      if (!input.data('ac-id')) {
        ...
        if (typeof settings.static !== 'undefined') {
          ...
          if (typeof settings.static.groups !== 'undefined') {
            for (let i = 0; i < settings.static.groups.length; i++) {
