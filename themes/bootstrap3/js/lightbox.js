...
VuFind.register('lightbox', function Lightbox () {
  // State
  let _originalUrl = false
  let _currentUrl = false
  let _lightboxTitle = false
  const refreshOnClose = false
  let _modalParams = {}
  // Elements
  let _modal; let _modalBody; let _clickedButton = null
  // Utilities
  ...
  function _html (content) {
    ...
    const $h2 = _modalBody.find('h2:first-of-type')
  ...
  function render (content) {
    ...
    // Isolate successes
    const htmlDiv = $('<div/>').html(content)
    const alerts = htmlDiv.find('.flash-message.alert-success:not([data-lightbox-ignore])')
    if (alerts.length > 0) {
      const msgs = alerts.toArray().map(function getSuccessHtml (el) {
        return el.innerHTML
      }).join('<br/>')
      const href = alerts.find('.download').attr('href')
    ...
    const forms = _modalBody.find('form:not([data-lightbox-ignore])')
    ...
    _xhr.always(function lbAjaxAlways () { _xhr = false })
      .done(function lbAjaxDone (content, status, jq_xhr) {
        ...
        if (jq_xhr.status === 204) {
          ...
          const testDiv = $('<div/>').html(content)
          ...
          if (obj.url.match(/\/Record\/.*(Hold|Request)\?/)) {
            if (errorMsgs.length && testDiv.find('.record').length) {
              const msgs = errorMsgs.toArray().map(function getAlertHtml (el) {
                return el.innerHTML
              }).join('<br/>')
        ...
        if (
          ...
          const eventResult = _emit('VuFind.lightbox.login', {
  ...
  _constrainLink = function constrainLink (event) {
    ...
    if (this.href.length > 1) {
      ...
      const obj = { url: $(this).data('lightbox-href') || this.href }
      if (typeof $(this).data('lightbox-post') === 'string') {
        obj.type = 'POST'
  ...
  _formSubmit = function formSubmit (event) {
    ...
    const submit = $(_clickedButton)
    _clickedButton = null
    const buttonData = { name: 'submit', value: 1 }
    if (submit.length > 0) {
    ...
    // Special handlers
    // On submit behavior
    if (typeof $(form).data('lightboxOnsubmit') === 'string') {
      const ret = _evalCallback($(form).data('lightboxOnsubmit'), event, data)
    ...
    // onclose behavior
    if (typeof $(form).data('lightboxOnclose') === 'string') {
  ...
  function getFocusableNodes () {
    const nodes = _modal[0].querySelectorAll(FOCUSABLE_ELEMENTS)
  ...
  function setFocusToFirstNode () {
    ...
    const nodesWhichAreNotCloseTargets = focusableNodes.filter(function nodeFilter (node) {
  ...
  function bind (el) {
    ...
    $('[data-lightbox-image]', el).each(function lightboxOpenImage (i, link) {
      ...
      $(link).bind('click', function lightboxImageRender (event) {
        event.preventDefault()
        const url = link.dataset.lightboxHref || link.href || link.src
        const imageCheck = $.ajax({
          url: url,
          method: 'HEAD'
        })
