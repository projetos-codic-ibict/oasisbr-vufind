...
VuFind.register('itemStatuses', function ItemStatuses () {
  ...
  function displayItemStatus (result, $item) {
    ...
    if (typeof (result.error) !== 'undefined' &&
      ...
      for (let x = 0; x < result.locationList.length; x++) {
  ...
  const ItemStatusHandler = {
    ...
    checkItemStatusDone: function checkItemStatusDone (response) {
      ...
      for (let j = 0; j < data.statuses.length; j++) {
        const status = data.statuses[j]
        this.items[status.id].result = status
        this.items[status.id].state = 'done'
        for (let e = 0; e < this.items[status.id].elements.length; e++) {
  ...
  const OdItemStatusHandler = Object.create(ItemStatusHandler)
  ...
  function checkItemStatus (el) {
    ...
    const id = $item.find('.hiddenId').val()
    let handlerName = 'ils'
  ...
  function checkItemStatuses (_container) {
    ...
    const ajaxItems = $(container).find('.ajaxItem')
