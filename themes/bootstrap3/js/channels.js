...
VuFind.register('channels', function Channels () {
  function addLinkButtons (elem) {
    const links = JSON.parse(elem.dataset.linkJson)
    if (links.length === 0) {
      return
    }
    const $cont = $(
    ...
    const $list = $('<ul class="dropdown-menu"></ul>')
      ...
      if (!record.data('popover-loaded')) {
        ...
        $.ajax({
          ...
          .done(function channelPopoverDone (data) {
            const newContent = '<div class="btn-group btn-group-justified">' +
  ...
  let bindChannelAddMenu // circular dependency fix for jshint

  function selectAddedChannel (e) {
    $.ajax(e.target.href).done(function addChannelAjaxDone (data) {
      const list = $(e.target).closest('.dropdown-menu')
      const $testEls = $('<div>' + data + '</div>').find('.channel-wrapper')
      const $dest = $(e.target).closest('.channel-wrapper')
  ...
  bindChannelAddMenu = function bindChannelAddMenuFunc (iteration, channel) {
    const scope = $(channel).parent('.channel-wrapper')
    ...
    $(scope).find('.channel-add-menu .add-btn').click(function addChannels (e) {
      const links = $(e.target).closest('.channel-add-menu').find('.dropdown-menu a')
